...
<section>
	...
	<ul>
		...
		if (isset($_POST['save'])) {
			...
			$artist = $link_new->query("SELECT id FROM {$_GLOBALS['DB_NAME_OLD']}.Brugere WHERE stutteri = '{$latin_1_name}' LIMIT 1")->fetch_object()->id;
			if ($artist && in_array((int) $status, [22, 26, 19, 25, 24]) && $race && is_numeric($id)) {

				$sql = "UPDATE {$_GLOBALS['DB_NAME_NEW']}.horse_types SET artists = '{$artist}', status = {$status}, race = '{$race}' WHERE id = {$id}";
//				echo $sql;
				$link_new->query($sql);
			}
		}

		$sql = "SELECT * FROM {$_GLOBALS['DB_NAME_NEW']}.horse_types WHERE (`status` IS NULL OR `race` IS NUll) AND id > 7000";
	...
	<datalist id="horse_races">
		<?php
		$result = $link_new->query("SELECT name FROM {$_GLOBALS['DB_NAME_NEW']}.horse_races");
		while ($data = $result->fetch_object()) {
