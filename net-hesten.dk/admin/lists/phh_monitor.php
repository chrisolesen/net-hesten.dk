...
<section>
    ...
    (SELECT GROUP_CONCAT(DISTINCT LEAST(`buyer`,`seller`),' ', GREATEST(`seller`,` buyer`)) FROM `game_data_private_trade` 
    WHERE (`seller` = `seller_id` AND `buyer` = `buyer_id`) OR (`seller` = `buyer_id` AND `buyer` = `seller_id`)) AS `pair`  
