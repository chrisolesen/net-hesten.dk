...
foreach ($intervals as $interval) {
	...
	$players = $link_new->query("SELECT `user_id`, SUM(TIME_TO_SEC(TIMEDIFF(`end`, `start`))) AS `duration` 
    FROM `{$GLOBALS['DB_NAME_NEW']}`.`user_data_sessions` WHERE `start` > '{$start}' AND `start` < '{$end}' 
    GROUP BY `user_id` ORDER BY (SUM(TIME_TO_SEC(TIMEDIFF(`end`, `start`)))) 
    DESC LIMIT 20");
