	...
	<div style="float:left;width: 650px;">
		<div class="image_block" style="height:130px;float:left;overflow: hidden;">
			...
			if ($visit_user_info->thumb) {
				$stud_thumbnail = "https://files.net-hesten.dk/users/{$visit_user_info->thumb}";
			} else {
		...
		<span class="label">Antal heste:</span><?= number_dotter($link_new->query("SELECT count(id) AS amount FROM {$_GLOBALS['DB_NAME_OLD']}.Heste WHERE bruger = '{$visit_user_info->username}' AND status <> 'død'")->fetch_object()->amount); ?><br />
