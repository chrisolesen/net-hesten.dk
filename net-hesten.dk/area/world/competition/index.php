	...
	<section>
		<div class="grid">
			<div data-section-type="info_square">
				<header><h1>Stævner og kårringer</h1></header>
			</div>
			<?php

			$date_now = new DateTime('NOW');
			if ($date_now->format('Y-m-d H:i:s') > '2018-04-12 17:55:00') {
				$sql = "SELECT * FROM `game_data_competition_participants` WHERE `competition_id` < 57 AND points IS NULL";
				$result = $link_new->query($sql);
				$competitions = [];
				while ($data = $result->fetch_object()) {
					$competitions[$data->competition_id][] = $data->participant_id;
				}
				foreach ($competitions as $competion_id => &$competition) {

					shuffle($competition);
				}
				foreach ($competitions as $competion_id => $competition) {
					foreach ($competition as $point => $participant_id) {
						$points = $point + 1;
						$link_new->query("UPDATE `game_data_competition_participants` SET points = '{$points}' WHERE `competition_id` = {$competion_id} AND `participant_id` = {$participant_id}");
					}
				}
			}
			...
			while ($data = $result->fetch_object()) {
				...
				<div data-section-type="competition">
					...
					<?php if ($data->name == 'Følkåring') { ?>
						<div style="text-align: initial;"><h3>Kåringer gives for:</h3>Helhedsindtryk, Kropsbygning, Temperament og Gangart<br /></div>
					<?php } ?>
					<?php if ($data->name != 'Følkåring') { ?>
			...
			jQuery.get({url: "https://ajax.net-hesten.dk/index.php?user_id=<?= $_SESSION['user_id']; ?>&request=suggest_event_participants&limits=" + jQuery(caller).attr('data-limits'), cache: false}).then(function (data) {
				...
				jQuery('[data-type="potential_breed_target"]').each(function () {
					jQuery(this).click(function () {
						...
						//						jQuery(this).parent().parent().parent().find('input[type="submit"]').removeProp('disabled');
					});
				});
			});
