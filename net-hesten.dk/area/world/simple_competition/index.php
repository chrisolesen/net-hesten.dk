...
$competitions = $link_new->query("SELECT * FROM {$_GLOBALS['DB_NAME_NEW']}.game_data_simple_competition WHERE startdate < NOW() ORDER BY startdate DESC LIMIT 2");
if (filter_input(INPUT_GET, 'partake') == true) {
	$part_id = (int) filter_input(INPUT_GET, 'comp_id');
	if($part_id){
		$link_new->query("INSERT INTO {$_GLOBALS['DB_NAME_NEW']}.game_data_simple_competition_participants (competition_id, participant_id) VALUES ({$part_id}, {$_SESSION['user_id']})");
	}
}
?>
<style>
	...
	<div class="grid">
		...
		<div class="drawing">
			...
			<?php if($competion_data->winner <> 0) { 
				$competition_winner = $link_new->query("SELECT stutteri AS username FROM {$_GLOBALS['DB_NAME_OLD']}.Brugere WHERE id = {$competion_data->winner}")->fetch_object();
				?>
			...
			<?php if ($competion_data->enddate > ((new DateTime('NOW'))->format('Y-m-d H:i:s')) && $competion_data->status_code == 42) { ?> 
				<?php 
				$partaking = false;
				$partaking = $link_new->query("SELECT * FROM {$_GLOBALS['DB_NAME_NEW']}.game_data_simple_competition_participants WHERE competition_id = {$competion_data->id} AND participant_id = {$_SESSION['user_id']}");
