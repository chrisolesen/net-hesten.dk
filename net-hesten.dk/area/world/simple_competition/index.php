...
?>
<style>
	.competition_data > div {line-height:1.5;}
	.competition_data > div span {font-weight:bold;}
	.drawing {
		display:block;
		clear:both;
	}
...
<section class="tabs">
	<div class="grid">
		<div data-section-type="info_square">
			<header><h1>Lodtrækninger</h1></header>
		</div>
		...
		<?php while($competion_data = $competitions->fetch_object()) {
			...
			?>
		<div class="drawing">
			<img style="float:left;margin-right:20px;" src="//<?= filter_input(INPUT_SERVER,'HTTP_HOST');?>/imgHorse/<?= $link_new->query("SELECT image FROM `{$GLOBALS['DB_NAME_NEW']}`.horse_types WHERE id = {$competion_subdata->prize_id}")->fetch_object()->image; ?>" />
			<div style="float:left;" class="competition_data">
			<h2><?= $competion_data->competition_name; ?></h2>
			<div><span>Race:</span> <?= $link_new->query("SELECT race FROM `{$GLOBALS['DB_NAME_NEW']}`.horse_types WHERE id = {$competion_subdata->prize_id}")->fetch_object()->race; ?></div>
			...
			<?php if($competion_data->winner <> 0) { 
				$competition_winner = $link_new->query("SELECT stutteri AS username FROM `{$GLOBALS['DB_NAME_OLD']}`.Brugere WHERE id = {$competion_data->winner}")->fetch_object();
				?>
			...
			<?php if ($competion_data->enddate > ((new DateTime('NOW'))->format('Y-m-d H:i:s')) && $competion_data->status_code == 42) { ?> 
				<?php 
				$partaking = false;
				$partaking = $link_new->query("SELECT * FROM `{$GLOBALS['DB_NAME_NEW']}`.game_data_simple_competition_participants WHERE competition_id = {$competion_data->id} AND participant_id = {$_SESSION['user_id']}");
