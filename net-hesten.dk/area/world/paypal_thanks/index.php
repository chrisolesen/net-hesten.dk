...
//	Så "skjul mit navn" / "skjul mit beløb men behold plads på listen" / "skjul navn og beløb"
	$donors = $link_new->query("SELECT user.id AS id, user.stutteri AS user, sum(payment.price) AS amount, MAX(payment.date) AS last_date FROM {$_GLOBALS['DB_NAME_OLD']}.PaypalPayment AS payment LEFT JOIN {$_GLOBALS['DB_NAME_OLD']}.Brugere AS user ON user.id = payment.stud_id WHERE payment.date > '2015-08-01 00:00:00' GROUP BY payment.stud_id ORDER BY amount DESC, last_date DESC");
	$donors_formatted = [];
