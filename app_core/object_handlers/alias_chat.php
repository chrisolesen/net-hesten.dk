...
class alias_chat
{
	...
	public static function post_message($attr = [])
	{
		...
		$sql = "INSERT INTO `game_data_alias_chat` (`creator`, `alias`, `value`) VALUES ({$attr['poster_id']}, '{$attr['alias']}', '{$attr['message']}')";
	...
	public static function get_messages($attr = [])
	{
		...
		$sql = "SELECT `new`.`creation_date`, `new`.`value`, `new`.`alias_id` 
		FROM `{$GLOBALS['DB_NAME_NEW']}`.`game_data_alias_chat` AS `new` 
