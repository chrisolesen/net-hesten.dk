...
class auctions
{
	...
	public static function list_bids($attr = [])
	{
		...
		$sql =  "SELECT `a`.`object_id` AS `horse_id`, `a`.`status_code` AS `auction_status`, `b`.`bid_amount`, `b`.`bid_date`,
		 `b`.`status_code` AS `bid_status`, `b`.`auction`, `b`.`creator` 
	...
	public static function get_highest_bid($attr = ['auction_id'])
	{
		...
		$sql = "SELECT `highest_bid`, `highest_bidder` 
	...
	public static function place_bid($attr = [])
	{
		...
		$auction_data = false;
		$result = $link_new->query("SELECT `id`, `object_id`, `creator`, `status_code`, `minimum_price`, `instant_price`, `highest_bidder`, `highest_bid`, `end_date` 
		FROM `{$GLOBALS['DB_NAME_NEW']}`.`game_data_auctions` 
		WHERE `id` = {$attr['auction_id']} AND `status_code` = 1 
		LIMIT 1");
		if ($result) {
		...
		if ($auction_data) {
			...
			if ($attr['mode'] == 'buy_now') {
				...
				$sql = "SELECT `stutteri`, `penge` 
				FROM `{$GLOBALS['DB_NAME_OLD']}`.`Brugere` WHERE `id` = {$_SESSION['user_id']} 
				LIMIT 1";
				$result = $link_new->query($sql);
				if ($result) {
				...
				if (is_null($auction_data['highest_bid'])) {
					$link_new->query("UPDATE `game_data_auctions` 
					SET `highest_bidder` = {$_SESSION['user_id']}, `highest_bid` = {$auction_data['instant_price']} 
					WHERE `id` = {$attr['auction_id']} AND `status_code` = 1 
					LIMIT 1");
				} else {
					$link_new->query("UPDATE `game_data_auctions` 
					SET `highest_bidder` = {$_SESSION['user_id']}, `highest_bid` = {$auction_data['instant_price']} 
					WHERE `highest_bidder` = {$auction_data['highest_bidder']} AND `highest_bid` = {$auction_data['highest_bid']} AND `id` = {$attr['auction_id']} AND `status_code` = 1 
					LIMIT 1");
				}

				if (mysqli_affected_rows($link_new) != 0) {
					if (!is_null($auction_data['highest_bid'])) {
						$link_new->query("UPDATE `game_data_auction_bids` 
						SET `status_code` = 5 
						WHERE `bid_amount` = {$auction_data['highest_bid']} AND `creator` = {$auction_data['highest_bidder']} AND `auction` = {$auction_data['id']}");
					...
					$link_new->query("INSERT INTO `game_data_auction_bids` (`creator`, `auction`, `bid_amount`, `bid_date`, `status_code`) 
					VALUES ({$_SESSION['user_id']}, {$attr['auction_id']}, {$auction_data['instant_price']}, '{$bid_date->format('Y-m-d H:i:s')}', 4)");
			...
			if ($attr['mode'] == 'place_bid') {
				...
				if (is_null($auction_data['highest_bid'])) {
					$link_new->query("UPDATE `game_data_auctions` SET `highest_bidder` = {$_SESSION['user_id']}, `highest_bid` = {$attr['bid_amount']} 
					WHERE `id` = {$attr['auction_id']} AND `status_code` = 1 
					LIMIT 1");
				} else {
					$link_new->query("UPDATE `game_data_auctions` SET `highest_bidder` = {$_SESSION['user_id']}, `highest_bid` = {$attr['bid_amount']} 
				...
				if (mysqli_affected_rows($link_new) != 0) {
					if (!is_null($auction_data['highest_bid'])) {
						$link_new->query("UPDATE `game_data_auction_bids` SET `status_code` = 5 
						WHERE `bid_amount` = {$auction_data['highest_bid']} AND `creator` = {$auction_data['highest_bidder']} AND `auction` = {$auction_data['id']}");
					...
					$link_new->query("INSERT INTO `game_data_auction_bids` (`creator`, `auction`, `bid_amount`, `bid_date`, `status_code`) 
					VALUES ({$_SESSION['user_id']}, {$attr['auction_id']}, {$attr['bid_amount']}, '{$bid_date->format('Y-m-d H:i:s')}', 4)");
