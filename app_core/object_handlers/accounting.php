...
class accounting
{
	...
	public static function fetch_entries($attr = [])
	{
		...
		$sql = "SELECT `entry_date`, `line_meta`, `line_amount` 
		FROM `{$GLOBALS['DB_NAME_NEW']}`.`game_data_accounting` 
	...
	public static function add_entry($attr = [])
	{
		...
		$meta_for_db = str_replace("'","\\'",json_encode($attr['meta_data'], JSON_UNESCAPED_UNICODE));
