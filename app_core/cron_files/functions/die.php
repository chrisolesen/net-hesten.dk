...
while ($horse = $result->fetch_object()) {
    ...
    if (mt_rand(0, 100) <= $horse->alder) {
        ...
        $user = $link_new->query("SELECT id, penge, stutteri FROM {$GLOBALS['DB_NAME_OLD']}.Brugere WHERE stutteri = '{$horse->bruger}' LIMIT 1")->fetch_object();
        $kids = $link_new->query("SELECT count(id) AS kids FROM {$GLOBALS['DB_NAME_OLD']}.Heste WHERE farid = {$horse->id} OR morid = {$horse->id}")->fetch_object()->kids;
        ...
        /* $sql = "UPDATE {$GLOBALS['DB_NAME_OLD']}.Brugere SET penge = (penge + {$claim}) WHERE id = {$user->id}";
        $link_new->query($sql); */
