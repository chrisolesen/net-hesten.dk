...
while ($competition_data = $competition_result->fetch_object()) {
	if ($end_competition_id = $competition_data->id) {
		...
		$competition = $link_new->query("SELECT `status_code`, `name`, `end_date` FROM `{$GLOBALS['DB_NAME_NEW']}`.`game_data_competitions` WHERE `id` = {$end_competition_id}")->fetch_object();
		if ($competition->status_code == 31) {
			...
			while ($data = $result->fetch_object()) {
				...
				if (in_array($points, $point_array)) {
					...
					$message = "<b>Tilykke {$data->uname}</b>,<br /><br /> Din hest {$data->hname} har vundet {$medal} i {$competition->name}. ({$competition->end_date})<br /><br />Du har fået {$price_money}wkr og din hest er steget med {$value_add} i værdi.<br /><br /><b>Med venlig hilsen</b><br />Konkurrencestyrelsen";
					...
					$link_new->query("INSERT INTO `{$GLOBALS['DB_NAME_NEW']}`.`game_data_private_messages` (`status_code`, `hide`, `origin`, `target`, `date`, `message`) VALUES (17, 0, {$origin}, {$data->uid}, NOW(), '{$message}' )");
				}
