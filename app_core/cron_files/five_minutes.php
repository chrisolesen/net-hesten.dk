...
$log_content = '';
$sql = "SELECT `auctions`.`*`, `bids`.`auction` FROM `{$GLOBALS['DB_NAME_NEW']}`.`game_data_auctions` AS `auctions` "
	...
	while ($data = $result->fetch_assoc()) {
		...
		if ($data['object_type'] == 1) {
			...
			$sql = "UPDATE `{$GLOBALS['DB_NAME_OLD']}`.`Heste` AS `h` "
				...
				. "WHERE `h`.`id` = {$data['object_id']} "
				. "AND `h`.`bruger` = 'Auktionshuset'";
...
$sql = "SELECT `auctions`.`id` AS `auction_id`, `auctions`.`creator` AS `seller`, `auctions`.`object_id` AS `object_id`, "
	. "`bids`.`creator` AS `winner`, `bids`.`bid_amount` AS `winning_amount`, `bids`.`bid_date` AS `bid_date` "
