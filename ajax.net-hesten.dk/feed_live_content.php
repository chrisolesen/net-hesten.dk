...
if (($user_id = (int) filter_input(INPUT_GET, 'user_id'))) {
	...
	$last_main_chat_message = $link_new->query("SELECT `creation_date` FROM `{$GLOBALS['DB_NAME_NEW']}`.`game_data_chat_messages` ORDER BY `creation_date` DESC LIMIT 1;")->fetch_object()->creation_date;
	$last_main_chat_read = $link_new->query("SELECT `value` FROM `{$GLOBALS['DB_NAME_NEW']}`.`user_data_timing` WHERE `name` = 'last_online_chat' AND `parent_id` = {$user_id} LIMIT 1")->fetch_object()->value;
